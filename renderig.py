...
CHECK_INTERVAL = 0.4               # small sleep inside per-thread watcher to avoid busy-loop
THREAD_FETCH_INTERVAL = 90        # 90 sec
WELCOME_MESSAGE = "welcome to Spyther's bot type help to see available commands ⚡"
MAX_MESSAGES_TRACK = 200
MAX_HISTORY_MESSAGES = 10  # max user+assistant pairs

GC_WELCOMES = [
    "👋 Spyther bot aapka swagat karta hai is GC mein! Type help for commands ⚡",
    "🎉 Welcome aboard! Spyther's bot is here to assist. Check help for magic ✨",
    "🚀 Namaste! Join the fun with Spyther bot. help to explore features 🌟"
]
...
def send_qr(cl: Client, thread_id: str):
    ...
    try:
        ...
        try:
            cl.direct_send_photo("qr.jpg", thread_ids=[int(thread_id)])
...
def get_ai_response(thread_id: str, sender_id: str, query: str, cl: Client) -> str:
    ...
    with history_lock:
        ...
        if not messages or messages[0]['role'] != 'system':
            messages.insert(0, {
                'role': 'system',
                'content': "You are Spyther's helpful bot named Spyther-Bot. Respond helpfully and conversationally."
            })
    ...
    try:
        if text == 'start':
            ...
            if len(parts) == 2 and parts[1].strip():
                username_to_fetch = parts[1].strip()
